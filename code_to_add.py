...
class CustomDialog(QDialog):
    ...
    def set_table(self, all_items,size):
...
class items:
    ...
    def create_dict_items(cls):
        ...
        cursor.execute("select item_name,item_price,item_stock,item_id from application.items ")
        for x in cursor:
    ...
    def get_items_names(cls):
        l=[]
        cursor.execute("select item_name from application.items ")
        for x in cursor:
            l.append(x[0])
...
def generate_bill(filename,bill_id,client_name,phone_number,all_bills,all_commande,type,remize):
    doc = SimpleDocTemplate(filename, pagesize=letter,leftMargin=28.35, rightMargin=28.35, topMargin=28.35, bottomMargin=28.35)
    story = []
...
def test_time(test_str):
    ...
    try:
        ...
        res = False
    return res
def confirm_update_items_date(old_date,commande_dict_date,new_date,all_dates,cin,index,type,phone_number,name):
    ...
    if type=="date":
        ...
        if test==False :
            ...
            try:
                ...
                select_client_profile(None, None, int(cin), name, phone_number)
...
def update_item_date_frame(existing_date,commande_dict,cin,name,phone_number,type,reteur_type):
    ...
    confirm.clicked.connect(lambda : confirm_update_items_date(old_date_list,commande_dict,new_date_entry.text(),existing_date,cin,commande_list.currentIndex(),type,phone_number,name))
...
def confirm_update_money_date(old_date,new_money,cin,name,phone_number):
...
def imprimer_facture_detailler(all_bills,all_commande,old_date_index,second_date_index,name,phone_number,remize_money,first_date,second_date):
    ...
    if date2 < date1:
        ...
        all_bills=all_bills[old_date_index:second_date_index+1]
        all_commande=all_commande[old_date_index:second_date_index+1]
...
def imprimer_facture(all_bills,all_commande,old_date_index,second_date_index,name,phone_number,remize_money,first_date,second_date):
    date1 = datetime.strptime(first_date.split(" : ")[0], '%Y-%m-%d')
    date2 = datetime.strptime(second_date.split(" : ")[0], '%Y-%m-%d')
    ...
    if date2<date1:
        ...
        generate_total_bill(
            ...
            all_items_total_quantiter, all_money_payed, remize_money)
...
def chosing_date_for_facture_frame(existing_date,all_bills,all_commande,cin,name,phone_number,remize_money,reteur_type):
    ...
    second_date_list = QComboBox()
    second_date_list.setStyleSheet(ch)
    second_date_list.addItems(existing_date)
    ...
    imprimer_facure_detailler.clicked.connect(
        lambda: imprimer_facture_detailler(all_bills, all_commande, old_date_list.currentIndex(),
                                           second_date_list.currentIndex(), name, phone_number, remize_money,old_date_list.currentText(),second_date_list.currentText()))
    ...
    imprimer_facure.clicked.connect(
        lambda: imprimer_facture(all_bills, all_commande, old_date_list.currentIndex(), second_date_list.currentIndex(), name, phone_number, remize_money,old_date_list.currentText(),second_date_list.currentText()))
...
def update_item_money_frame(existing_date,cin,name,phone_number,reteur_type):
    ...
    confirm=create_button("Confirm ")
    widgets["confirm"].append(confirm)
    confirm.clicked.connect(lambda : confirm_update_money_date(old_date_list.currentText(),new_date_entry.text(),cin,name,phone_number))
...
def confirm_change_client_commande_date(first_date,new_date,ids,cin,name,phone_number):
    id='id'
    test = test_time(new_date)
    if test == False:
        QMessageBox.critical(None, 'Fail', "the date is not valid ")
        ...
        select_client_profile(None, None, int(cin), name, phone_number)
def confirm_annuler(date_to_delete,cin,name,phone_number):
    dialog = CustomDialog()
...
def annuler_commande_frame(all_dates,cin,name,phone_number,reteur_type):
    ...
    confirm=create_button("Confirm")
    confirm.clicked.connect(lambda : confirm_annuler(old_date_list.currentText(),cin,name,phone_number))
    widgets["confirm"].append(confirm)
...
def confirm_remise(remise, get_remize,cin, name, phone_number,all_bills,all_commande,type):
    if str(remise).isdigit():
        get_remize[0]=int(remise)
        QMessageBox.information(None, "Success", "remize est confirmer")
        clear_widgets()
        client_bills_frame(all_bills,all_commande,type,name,phone_number,cin,int(remise))

    else :
        QMessageBox.critical(None, 'Fail', "remize have to be digit ")
def remise_fn(get_remize,cin,name,phone_number,all_bills,all_commande,type):
    clear_widgets()
    remise_frame(get_remize,cin,name,phone_number,all_bills,all_commande,type)
def remise_frame(get_remize,cin,name,phone_number,all_bills,all_commande,type):
    ch = "border : 1px solid '#5B2A86';" + "border-radius: 15px;" + "font-size: " + font_size + "px;" + "color:'#5B2A86' ;" + "padding: 10px 0;" + "margin: 10px 10px ;"
    back = create_button("Reteur")
    widgets["button"].append(back)
    back.clicked.connect(lambda: select_client_profile(None, None, int(cin), name, phone_number))
    ...
    confirm=create_button("Confirm")
    confirm.clicked.connect(lambda : confirm_remise(remise.text(),get_remize,int(cin), name, phone_number,all_bills,all_commande,type))
    widgets["confirm"].append(confirm)
        ...
        if item_pricee.isdigit():
            ...
            if k == 1:
             if int(item_pricee)>0:
                cursor.execute(f"update application.items set item_price={item_pricee} where item_name='{item_name}'")
                connection.commit()
        ...
        if item_stockk.isdecimal():
            ...
            if k == 1:
                if float(item_stockk) >= 0:
                    cursor.execute(
                        f"update application.items set item_stock=item_stock-{item_stockk} where item_name='{item_name}'")
                    connection.commit()
        ...
        if item_name != "":
            if item_stockk.isdecimal():
                ...
                if k == 1:
                    if float(item_stockk) >= 0:
                        cursor.execute(
                            f"update application.items set item_stock={item_stockk} where item_name='{item_name}'")
                        connection.commit()
                        QMessageBox.information(None, "Succes", "Le stock d'articles a été mis à jour avec succès  ")
                        item_stock.setText("")
                    else:
            ...
            QMessageBox.critical(None, 'Fail', "Please enter Item name  ")
def confirm_create_client_bill(full_name,cin,date_entry,table,all_items,money_entry,welcome_label,type):
    print(all_items)
    ...
    if moneyy== "" :
        ...
        if not moneyy[1:].isdigit():
            moneyy= "0"
    print(moneyy)
    text=f"client : {full_name} Tel: {cin} reglement : {money(moneyy)}  date : {date} \n"
    size=0
    print(text)
    test_2=False
    test = test_time(date)
    if test==False:
        ...
        dialog.setText(long_text)
        dialog.set_table(list(all_items.values()),size)
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            if test_2:
                if type!="devis":
                    items = the_client.create_clent_bill_commande(list(all_items.values()), date, moneyy)
                else:
...
def confirm_create_supplier_bill(full_name,cin,calender,table,itemss,money_entry,welcome_entry):
    full_name=str(full_name.text())
    cin=str(cin.text())
    if cin=="" or  not (cin.isdigit()):
        ...
        if test == False:
            ...
            for i, item in enumerate(itemss):
                lineEdit = table.cellWidget(i +1, 2)
                if isinstance(lineEdit, QLineEdit):
                    quantiter_text = lineEdit.text()
                    print(quantiter_text)
                    if quantiter_text=="":
                        item.item_quantiter = "0"

                    elif (quantiter_text[0] == "-" and quantiter_text[1:].isdigit()) or quantiter_text.isdigit() or \
                            quantiter_text[0].isdecimal():
                        item.item_quantiter = quantiter_text
                        print(quantiter_text)

                item_price = str(table.item(i+1, 3).text())
                if item_price == "":
                    item_price = "0"
                elif item_price.isdigit():
                    item_price = str(table.item(i+1, 3).text())
            ...
            print("items[1].price",itemss[1].price)
...
def client_bills_fn(all_bills,all_commande,type,name,phone_number,cin):
    clear_widgets()
    client_bills_frame(all_bills,all_commande,type,name,phone_number,cin,0)
def update_stock_fn():
    clear_widgets()
    update_stock_frame()
def update_price_fn():
...
def select_fn(calander,type):
    ...
    i=0
    if type=="all":
        cursor.execute(f"select b.client_bill_id,b.money_payed,c.cin,c.client_full_name,c.phone_number from client_bill b,the_client c where client_bill_date= '{selected_date}' and c.cin=b.cin ")
        for x in cursor:
...
def client_bills_frame(all_bills,all_commande,type,name,phone_number,cin,remize_money):
    ch = "border : 1px solid '#5B2A86';" + "border-radius: 15px;" + "font-size: "+font_size+"px;" + "color:'#5B2A86' ;" + "padding: 10px 0;" + "margin: 10px 10px ;"
    back = create_button("Reteur")
    widgets["button"].append(back)
    all_dates_money = []
    ...
    tableWidget.setItem(0, 4, QTableWidgetItem("Règlement"))
    tableWidget.setItem(0, 5, QTableWidgetItem("Date"))
    tableWidget.setItem(0,6,QTableWidgetItem("Commande code "))
    ...
    for commande in all_commande:
        ...
        for x in commande:
            i=i+1
            tableWidget.setItem(i,6,QTableWidgetItem(str(all_bills[j].client_bill_id)))
        ...
        tableWidget.setItem(i+1, 6, QTableWidgetItem(str(all_bills[j].client_bill_id)))
        all_money_payed=all_money_payed+int(all_bills[j].money_payed)
        i=i+1
        j=j+1
    tableWidget.setItem(i+1, 1, QTableWidgetItem("Total"))
    ...
    #tableWidget.doubleClicked.connect(lambda: select_worker(tableWidget, users, date))
    try:
        generate_bill("facture_detaillee\\facture "+make_windows_friendly_filename(name)+".pdf", str(all_bills[0].client_bill_id), name, phone_number, all_bills, all_commande,"bill",remize_money)
    except Exception as e:
        print(e)
        print(traceback.format_exc())
    ...
    choosing_date.clicked.connect(
        lambda: chosing_date_for_facture_fn(all_dates_with_code, all_bills,all_commande, cin, name, phone_number,remize_money,type))
    ...
    remize.clicked.connect(lambda: remise_fn(get_remize, cin, name, phone_number,all_bills,all_commande,type))
...
def change_date_frame(name,all_dates,cin,ids,phone_number,reteur_type):
    ...
    conferm.clicked.connect(
        lambda: confirm_change_client_commande_date(old_date.currentText(),new_date_entry.text(),ids,cin,name,phone_number))
    grid.addWidget(widgets["label"][-1], 1, 0)
    grid.addWidget(widgets["explain_label"][-1], 2, 0)
...
def create_client_bill_frame(type):
    ...
    if type!="reteur":
        conferm.clicked.connect(lambda: confirm_create_client_bill(full_name_entry,cin_entry,date_entry.text(),tableWidget,all_items_dict,money_entry,welcome_label,type))
    else:
        try:
...
def create_supplier_bill_frame():
    ...
    def updateInputsFromName(index):
        ...
        for thing in all_client:
            if thing.name == selected_name:
                cin_entry.setText(str(thing.id))
    ...
    conferm = create_button("Confirm ")
    widgets["confirm"].append(conferm)
    conferm.clicked.connect(lambda: confirm_create_supplier_bill(full_name_entry,cin_entry,date_entry.text(),tableWidget,all_items,money_entry,welcome_label))
   ...
   # tableWidget.doubleClicked.connect(lambda :select_worker(tableWidget,users,date))
    ...
    scroll_grid.addWidget(widgets["table"][-1], 3, 0,1,2)
    scroll_grid.addWidget(widgets["money_entry"][-1], 4, 1)
    scroll_grid.addWidget(widgets["confirm"][-1], 4, 0)
    scroll_grid.addWidget(widgets["see_total"][-1], 5, 0)
    scroll_grid.addWidget(widgets["button"][-1], 0, 0)
    scroll_grid.addWidget(widgets["label"][-1], 5, 1)
    grid.addWidget(widgets["scroll"][-1], 0, 0)
